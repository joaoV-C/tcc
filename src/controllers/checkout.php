<?php
function handleCheckoutRequest(): void {
  ...
  try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['subtotal'], $_POST['tax'], $_POST['total'])) {
      ...
      if (!empty($errors)) {
        ...
        header('Location: /tcc/checkout?compra=falha');
        exit();
      } else {
        if (isset($_SESSION['user_id'])) {
          ...
          header('Location: /tcc/checkout?compra=sucesso');
          exit();
        } else {
    ...
    if (empty($_SESSION['cart']) && empty($_SESSION['checkout_success'])) {
      header('Location: /tcc/cart?cesto=vazio');
      exit();
    }
  } catch (PDOException $e) {
    $_SESSION['checkout_errors'] = ("Falha no envio: " . $e->getMessage());
    throw $e;
    header("Location: /tcc/checkout?erro");
    exit();
  }
  ...
  include __DIR__ . '/../views/checkout_view.inc.php';
}
